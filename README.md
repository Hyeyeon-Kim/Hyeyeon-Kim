...
- 운영 10개월 동안 10만장의 사진을 요청을 하는 29,798건 동안 에러 없이 처리
...
|종류|설명|장점|단점|
|트리or계층형|트리 형태로 배치한 네트워크 구성|노드 추가, 삭제가 쉬움|특정 노드에 트래픽이 집중될 때 하위 노드에 영향을 끼칠 수 있음|
|버스|중앙 통신 회선 하나에 여러 개의 노드가 연결되어 공유|근거리 통신망에서 사용|
|스타|중앙에 있는 노드에 모두 연결됨|쉬운 노드 추가&에러를 탐지, 패킷의 충돌 발생 가능성 낮음, 에러 발생 시 다른 노드에 영향이 적음|중앙 노드에 장애가 발생 시 전체 네트워크를 사용 X, 설치 비용이 높음
|링형|전체적으로 고리처럼 양 옆의 노드가 연결되어 있음|노드 수가 증가되어도 네트워크 상의 손실이 적음, 충돌이 발생되는 가능성 낮음, 쉬운 고장 발견|네트워크 구성 변경이 어려움, 회선에 장애가 발생하면 전체 네트워크에 영향을 줌|
